...
void displayEphemeris(pulsar *psr,linkedList *list,int npsr,int *ptr,int np,paramtype *defParam,int ephem,int webflag)
{
  int i,j,ierr,ip,ip2,ip3,ip4,ip5,ip6,ip7,ip8,ip9,eph;
  char str[1000];
  pulsar *psrptr;
  parameter dispParam;
      ...
      if (ephem==2 || ephem==1 || ephem==4)
	{
	  for (j=0;j<MAX_PARAM;j++)
	      ...
	      if (ephem==2 || ephem==4 || defParam[j].ephem==1)
		{
	      ...
	      if (ephem==2 || ephem==4 || (defParam[j].ephem==1 && dispParam.derived!=1))
		{
		  //		  printf("j = %d\n",j);
		  if (strcmp(pcat_parameters[j],"C1")!=0 &&
		      strcmp(pcat_parameters[j],"C2")!=0 &&
  ...
  if (flag==4)
    {
      ...
      fclose(fout);
      system("\\rm dummy.bbl");
      system("/usr/local/texmf/bin/latex dummy.tex > dummy.out");
      system("/usr/local/texmf/bin/bibtex dummy >> dummy.out");
      printf("<pre>\n");
