  ...
  *badval = 0;
  do {   
	...
	if (psr->param[ip].set1!=1)
	  ...
	  {
	    if (strncasecmp(psr->param[ip].val,storeStr,strlen(storeStr))==0)
	      strcat(newstr,"1");
	    else
	      strcat(newstr,"0");
	  }
	...
	if ((ip = getParam("TYPE",NULL))==-1)
	  ...
	  strcat(newstr,"1");
									    ...
									    strcmp(psr->param[ip].val,"AXP")!=0 && 
									    strcmp(psr->param[ip].val,"NR")!=0)))
	  strcat(newstr,"1");
	...
	if (tempptr!=NULL) strcpy(storeStr, tempptr+1);
      }
...
void alwaysDerive(pulsar *psr,paramtype *defparam)
{
  ...
  if (psr->param[units].set1 == 0) /* Use UNITS TDB as default */
    {
